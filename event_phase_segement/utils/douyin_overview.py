...
def process_json_file(file_path):
    ...
    for comment in data.values():
        created_at = comment.get('time')  # 获取时间戳
        likes = comment.get('likes', 0)
        raw_comment = comment.get('content', "")
        
        # 将时间戳转换为 datetime 对象
        comment_time = datetime.fromtimestamp(created_at)
...
if __name__ == "__main__":
    input_directory = r"douyin\douyin\processed_data\json"  # 指定存放 JSON 文件的目录
    output_directory = r"event_phase_segement\douyin\weekly_details"  # 每周评论的 JSON 文件存放目录
    overview_file_path = r"event_phase_segement\douyin\weekly_stats.json"  # 总览统计结果文件路径
